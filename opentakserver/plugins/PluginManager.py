...
class PluginManager:
            ...
            try:
                try:
                    with self._app.app_context():
                        ...
                        plugin_row = db.session.execute(
                            db.session.query(Plugins).filter_by(name=plugin.name.lower())
                        ).first()
                        if plugin_row:
    ...
    def disable_plugin(self, plugin_distro: str):
        ...
        db.session.execute(
            update(Plugins).where(Plugins.name == plugin.name.lower()).values(enabled=False)
        )
        db.session.commit()
    ...
    def enable_plugin(self, plugin_distro: str):
        ...
        db.session.execute(
            update(Plugins).where(Plugins.name == plugin.name.lower()).values(enabled=True)
        )
        db.session.commit()
    ...
    def _add_plugin(self, plugin: Plugin) -> None:
        if not isinstance(plugin, Plugin):
            raise ValueError("The OTS plugin must be an instance of Plugin")
    ...
    def check_if_plugin_enabled(self, plugin_distro: str) -> bool:
        with self._app.app_context():
            plugin = db.session.execute(
                db.session.query(Plugins).filter_by(name=plugin_distro.lower())
            ).first()
            if plugin:
    ...
    def install_plugin(json: dict):
        ...
        if not plugin_valid:
            ...
            socketio.emit(
                ...
                {"success": False, "message": f"Invalid Plugin: {json.get('plugin_distro')}"},
                to=request.sid,
                namespace="/socket.io",
            )
            return
        ...
        if json.get("action") == "delete":
            ...
            if not os.path.exists(os.path.join(app.config.get("UPLOAD_FOLDER"), file_name)):
                ...
                socketio.emit(
                    "plugin_package_manager",
                    {"success": False, "message": f"Plugin file not found: {file_name}"},
                    namespace="/socket.io",
                    to=request.sid,
                    ignore_queue=True,
                )
            ...
            socketio.emit(
                "plugin_package_manager",
                {"success": False, "message": f"Invalid action: {json.get('action')}"},
                namespace="/socket.io",
                to=request.sid,
                ignore_queue=True,
            )
        ...
        try:
            ...
            socketio.emit(
                "plugin_package_manager",
                {"success": False, "message": f"Command failed: {e}"},
                namespace="/socket.io",
                to=request.sid,
                ignore_queue=True,
            )
        ...
        if return_code == 0:
            socketio.emit(
                "plugin_package_manager",
                {"success": True, "message": "Command completed successfully"},
                to=request.sid,
                namespace="/socket.io",
                ignore_queue=True,
            )
            ...
            if json.get("action") == "install":
                ...
                plugin = db.session.execute(
                    db.session.query(Plugins).filter_by(distro=json.get("plugin_distro").lower())
                ).first()
                if plugin:
            ...
            socketio.emit(
                "plugin_package_manager",
                {"success": False, "message": f"Command failed with return code: {return_code}"},
                to=request.sid,
                namespace="/socket.io",
                ignore_queue=True,
            )
