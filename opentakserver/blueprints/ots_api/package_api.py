...
def download_package(package_name, atak_version=None):
    ...
    return send_from_directory(
        os.path.join(app.config.get("OTS_DATA_FOLDER"), "packages"), secure_filename(package_name)
    )
...
def head_product_infz_with_version(atak_version: str):
    ...
    if os.path.exists(
        os.path.join(app.config.get("OTS_DATA_FOLDER"), "packages", atak_version, "product.infz")
    ):
        return jsonify({"success": True})
...
def get_product_infz():
    ...
    if not cert:
        return "", 401
    return send_from_directory(
        os.path.join(app.config.get("OTS_DATA_FOLDER"), "packages"), "product.infz"
    )
...
def create_product_infz(atak_version: str | None):
    ...
    if atak_version:
        ...
        product_infz_file = os.path.join(
            app.config.get("OTS_DATA_FOLDER"), "packages", atak_version, "product.infz"
        )
        product_inf_file = os.path.join(
            app.config.get("OTS_DATA_FOLDER"), "packages", atak_version, "product.inf"
        )
        os.makedirs(
            os.path.join(app.config.get("OTS_DATA_FOLDER"), "packages", atak_version), exist_ok=True
        )
            ...
            for package in packages:
                ...
                csv_writer.writerow(
                    [
                        ...
                        package.file_name,
                        package.icon_filename,
                        package.description,
                        package.apk_hash,
                        package.os_requirement,
                        package.tak_prereq,
                        package.file_size,
                    ]
                )
...
def get_repository_inf():
    ...
    versions = (
        Packages.query.distinct(Packages.atak_version)
        .where(Packages.atak_version is not None)
        .all()
    )
    if not versions:
        return "", 404
...
def add_package():
    ...
    request.files["apk"].save(
        os.path.join(app.config.get("OTS_DATA_FOLDER"), "packages", apk_filename)
    )
    ...
    existing_package = db.session.execute(
        db.session.query(Packages).filter_by(
            package_name=package.package_name, atak_version=package.atak_version
        )
    ).scalar()
    if existing_package:
        logger.warning(
            f"{package.name} version {package.version} for ATAK {package.atak_version} is already on the server and will be removed"
        )

        if os.path.exists(
            os.path.join(app.config.get("OTS_DATA_FOLDER"), "packages", existing_package.file_name)
        ):
            os.remove(
                os.path.join(
                    app.config.get("OTS_DATA_FOLDER"), "packages", existing_package.file_name
                )
            )
    ...
    try:
        ...
        db.session.execute(
            ...
            .where(Packages.package_name == package.package_name)
            .values(**package.serialize())
        )
        db.session.commit()
...
def edit_package():
    ...
    package = db.session.execute(
        db.session.query(Packages).filter_by(
            package_name=form.package_name.data, atak_version=form.atak_version.data
        )
    ).first()
    if not package:
    ...
    db.session.execute(
        sqlalchemy.update(Packages)
        .where(Packages.package_name == form.package_name.data)
        .where(Packages.atak_version == form.atak_version.data)
        .values(**package.serialize())
    )
    db.session.commit()
...
def delete_package():
    ...
    if not package_name:
        return (
            jsonify(
                {
                    "success": False,
                    "error": "Please provide the package name of the plugin to delete",
                }
            ),
            400,
        )
    ...
    package = package[0]
    if os.path.exists(
        os.path.join(app.config.get("OTS_DATA_FOLDER"), "packages", package.file_name)
    ):
