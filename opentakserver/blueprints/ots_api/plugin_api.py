...
def upload_plugin():
    if hasattr(app, "plugin_manager"):
                        ...
                        jsonify(
                            {
                                "success": False,
                                "error": gettext(
                                    "Server plugins must be zip, whl, or tar.gz files"
                                ),
                            }
                        ),
                        400,
        ...
        return jsonify({"success": False, "error": gettext("Plugins are disabled")}), 400
...
def get_plugin(plugin_name: str):
    if hasattr(app, "plugin_manager"):
        ...
        if plugin:
            ...
            plugin_metadata["enabled"] = app.plugin_manager.check_if_plugin_enabled(
                plugin.name.lower()
            )
                ...
                jsonify(
                    {
                        "success": False,
                        "error": gettext(
                            "Plugin %(plugin_name)s not found", plugin_name=plugin_name
                        ),
                    }
                ),
                404,
