...
def create_channel():
    ...
    if "url" in request.json.keys() and request.json.get("url"):
        try:
            settings = base64.urlsafe_b64decode(
    ...
    # Make the protobuf from settings in order to make the URL
        ...
        try:
            ...
            if "psk" in request.json.keys() and request.json.get("psk"):
                channel_settings.psk = base64.urlsafe_b64decode(
                    bleach.clean(request.json.get("psk"))
                )
            else:
...
def save_channel(channel_set: apponly_pb2.ChannelSet):
    for channel_settings in channel_set.settings:
        ...
        url = "https://meshtastic.org/e/#" + base64.urlsafe_b64encode(
            single_channel_set.SerializeToString()
        ).decode("utf-8")

        if channel_settings.downlink_enabled and channel_settings.name not in app.config.get(
            "OTS_MESHTASTIC_DOWNLINK_CHANNELS"
        ):
            app.config.get("OTS_MESHTASTIC_DOWNLINK_CHANNELS")[
                channel_settings.name
            ] = channel_settings.psk
        ...
        meshtastic_channel_settings.position_precision = (
            channel_settings.module_settings.position_precision
        )
        ...
        meshtastic_channel_settings.lora_sx126x_rx_boosted_gain = (
            channel_set.lora_config.sx126x_rx_boosted_gain
        )
    ...
    if "url" not in request.args.keys():
        return (
            jsonify(
                {
                    "success": False,
                    "error": gettext("Please provide the URL of the channel to delete"),
                }
            ),
            400,
        )
...
def generate_psk():
    return (
        jsonify({"success": True, "psk": base64.urlsafe_b64encode(os.urandom(32)).decode("ascii")}),
        200,
    )
