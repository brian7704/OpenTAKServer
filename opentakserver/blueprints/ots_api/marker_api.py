...
def add_marker():
    ...
    try:
        if "latitude" not in request.json.keys() or "longitude" not in request.json.keys():
            return (
                jsonify(
                    {"success": False, "error": gettext("Please provide a latitude and longitude")}
                ),
                400,
            )
                ...
                jsonify(
                    {
                        "success": False,
                        "error": gettext(
                            "Invalid latitude: %(latitude)s", latitude=request.json["latitude"]
                        ),
                    }
                ),
                400,
                ...
                jsonify(
                    {
                        "success": False,
                        "error": gettext(
                            "Invalid longitude: %(longitude)s", longitude=request.json["longitude"]
                        ),
                    }
                ),
                400,
        ...
        return (
            jsonify(
                {"success": False, "error": gettext("Failed to parse lat/lon: %(e)s", e=str(e))}
            ),
            400,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "error": gettext("Invalid UID. UIDs need to be in UUID4 format")}
            ),
            400,
        )
    ...
    if not marker.affiliation or not marker.battle_dimension:
        return (
            jsonify(
                {
                    "success": False,
                    "error": gettext("Invalid type: %(cot_type)s", cot_type=cot_type),
                }
            ),
            400,
        )
    ...
    try:
        ...
        point.location_source = (
            bleach.clean(request.json["location_source"])
            if "location_source" in request.json.keys()
            else ""
        )
        ...
        with app.app_context():
            ...
            cot = db.session.execute(
                insert(CoT).values(
                    ...
                    stale=point.timestamp + timedelta(days=1),
            ...
            marker = db.session.execute(db.session.query(Marker).filter_by(uid=marker.uid)).first()[
                0
            ]
        ...
        return (
            jsonify({"success": False, "error": gettext("Failed to parse data: %(e)s", e=str(e))}),
            400,
        )
...
def delete_marker():
    ...
    if not uid:
        return (
            jsonify(
                {
                    "success": False,
                    "error": gettext("Please provide the UID of the marker to delete"),
                }
            ),
            400,
        )
    ...
    with app.app_context():
        ...
        if not marker:
            return jsonify({"success": False, "error": gettext("Unknown UID")}), 404
        ...
        detail = ET.SubElement(event, "detail")
        ET.SubElement(
            detail, "link", {"relation": "p-p", "uid": marker.uid, "type": marker.cot.type}
        )
        ET.SubElement(
            detail,
            "_flow-tags_",
